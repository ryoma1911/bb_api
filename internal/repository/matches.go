...
func (d *DefaultRepository) GetMatch(db *sql.DB, query string) ([]map[string]interface{}, error) {
	...
	for rows.Next() {
		...
		var status string
		var link string
		if err := rows.Scan(&id, &date, &home, &away, &league, &stadium, &starttime, &status, &link); err != nil {
...
func (d *DefaultRepository) GetMatchAPI(db *sql.DB, query string) ([]map[string]interface{}, error) {
	rows, err := db.Query(query)
	if err != nil {
		return nil, fmt.Errorf("failed to fetch match: %w", err)
	}
	...
	for rows.Next() {
		...
		var starttime string
		var status string
		if err := rows.Scan(&id, &date, &home, &away, &league, &stadium, &starttime, &status); err != nil {
			return nil, fmt.Errorf("failed to scan match row: %w", err)
		}
		//試合情報をマップに格納、スライスに追加
		matches = append(matches, map[string]interface{}{
