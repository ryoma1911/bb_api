...
func TestGetMatchAPI(t *testing.T) {
	...
	t.Run("Success to get match", func(t *testing.T) {
		//クエリ実行でテーブルからデータが取得されていること
		todate := time.Now().Format("2006/01/02")
		query := "SELECT id, date, home, away, league, stadium, starttime, status FROM matches"

		//モックの結果を定義
		rows := sqlmock.NewRows([]string{"id", "date", "home", "away", "league", "stadium", "starttime", "status"}).
			AddRow(1, todate, "Yankees", "Red Sox", "セ・リーグ", "Yankee Stadium", "19:00", "Scheduled").
			AddRow(2, todate, "Dodgers", "Giants", "パ・リーグ", "Dodger Stadium", "18:30", "Scheduled")

			// モックの期待値を設定
		mock.ExpectQuery(query).WillReturnRows(rows)

		// 関数を実行
		result, err := repo.GetMatchAPI(db, query)
	...
	// 行のスキャン失敗パターン
	t.Run("Failed to scan", func(t *testing.T) {
		query := "SELECT id, date, home, away, stadium, starttime, status FROM matches"
